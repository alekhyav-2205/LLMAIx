    ...
    try:
        if file_path.endswith('.csv'):
            ...
            for index, row in enumerate(df.itertuples()):
                ...
                # SKIP PDF generation for CSV rows:
                merged_data.append(pd.DataFrame({'report': [text], 'filepath': [''], 'id': id}))
...
def download():
    ...
    if job.cancelled():
        ...
        if df['filepath'].eq('').any():
            flash("Note: No individual PDFs were generated for CSV rows — only extracted text is included in the CSV file.", "info")
